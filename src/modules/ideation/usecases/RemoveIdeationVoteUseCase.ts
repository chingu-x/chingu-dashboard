...
export class RemoveIdeationVoteUseCase {
  ...
  /**
   * Executes the use case to remove an ideation vote.
   *
   * @param {IdeationVoteProps} props - The properties required to remove an ideation vote.
  ...
  async execute(
    ...
    const [res, error] = await this.ideationService.removeIdeationVote({
      ...props,
      token,
      baseUrl,
    });
