...
async function VoyageDashboard({ teamId, searchParams }: VoyageDashboardProps) {
    const [user, error] = await getUser();

    // Extract mock date from URL
    const mockDate = searchParams.mockDate
        ? new Date(searchParams.mockDate as string)
        : undefined;
    ...
    const techStackList = techStackDatas.map((techStackData) => ({
        ...
        value: techStackData.teamTechStackItems
            .map((item) => item.name)
            .join(", "),
    }));
    ...
    return (
        <>
            <UserProvider user={user} mockDate={mockDate} />
                    ...
                    <div className="flex flex-col gap-y-4">
                            ...
                            <div className="flex w-full grow">
                                ...
                                >
                                    {techStackList.some(
                                        ...
                                        <TechStackStateContent
                                            contentObject={techStackList}
                                        />
                                    ) : null}
                                </DashboardWidget>
                            </div>
                        ...
                        >
                            {resourceList.length > 0 ? (
                                <ResourcesStateContent
                                    contentObject={resourceList}
                                />
                            ) : null}
                        </DashboardWidget>
                    </div>
